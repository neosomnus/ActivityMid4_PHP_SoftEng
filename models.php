...
function deleteAccountant($pdo, $accountant_id) {
	$deleteAccountantClient = "DELETE FROM client WHERE accountant_id = ?";
	$deleteStmt = $pdo->prepare($deleteAccountantClient);
...
function getClientByAccountant($pdo, $accountant_id) {
	...
	$sql = "SELECT 
				...
				CONCAT(accountant.first_name,' ',accountant.last_name) AS client_accountant
			FROM client
...
function updateClient($pdo, $first_name, $last_name, $service_requested, $client_id) {
	$sql = "UPDATE client
			SET first_name = ?,
                last_name = ?,
				service_requested = ?
			...
			";
	$stmt = $pdo->prepare($sql);
	$executeQuery = $stmt->execute([$first_name, $last_name, $service_requested, $client_id]);
